...
<div className="flex-1 flex flex-col items-center justify-center gap-12 mt-8">
  ...
  <MenuButton label="Home" onClick={() => { onSectionChange(0);     setMenuOpened(false);        reset3D(); 
    // resetOverlays();          
      // resetCamera();   
   
  
  }}
   className="text-white text-stroke-black text-6xl font-bold cursor-pointer hover:text-indigo-600 transition-colors" />
  <MenuButton label="Free Quote"   onClick={(event) => {
    spawnCoin(event);
    onSectionChange(1);
  }}
   className="text-white text-stroke-black text-6xl font-bold cursor-pointer hover:text-indigo-600 transition-colors" />
  <MenuButton label="Services" onClick={(event) => {
    spawnCoin(event);
    onSectionChange(2);
  }}
   className="text-white text-stroke-black text-6xl font-bold cursor-pointer hover:text-indigo-600 transition-colors" />
  <MenuButton label="Contact Page"  onClick={(event) => {
    spawnCoin(event);
    onSectionChange(3);
  }}
   className="text-white text-stroke-black text-6xl font-bold cursor-pointer hover:text-indigo-600 transition-colors" />
        ...
        {/* GOOGLE REVIEWS  for later in a different page*/}
